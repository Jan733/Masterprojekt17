...
geometry = gpd.points_from_xy(df_nodes_alt.longitude,
                              df_nodes_alt.latitude)  # Point Geometrie mithilfe der longitude und Latitude
geo_df = gpd.GeoDataFrame(df_nodes_alt,
                          geometry=geometry)  # Erstellung der Geopandas Tabelle in die die Geometry eingetragen wird.
#   print(df_nodes_alt.head().to_string())
df_nodes_alt = df_nodes_alt.drop(["Location"], axis=1)  # Löschen des Columns Location
df_nodes_alt.rename(columns={"ID": "node_id"},
                    inplace=True)  # Umbennen des Columns ID, damit bei mehreren Tabellen die ID unter demselben Column steht (vereinfacht das zusammenfügen bei Betrachtung eines Indexes)
...
df_relation_members_alt["Nodes"] = df_relation_members_alt["Nodes"].apply(
    ast.literal_eval)  # Zugriff auf die einzelnen Werte der Liste: "Members"
df_relation_members_alt_memb = \
df_relation_members_alt["Nodes"].apply(pd.Series).reset_index().melt(id_vars="index").dropna()[
    ["index", "value"]].set_index(
    "index")  # Die Members werden einzelnt untereinander geschrieben und einem neuen Index zugeordnet
df_relation_members = pd.merge(df_relation_members_alt["ID"], df_relation_members_alt_memb, right_index=True,
                               left_index=True).rename(
    columns={'value': 'member_id', 'ID': 'relation_id'})  # Die einzelnen Members werden der relation zugeorndet
df_relation_members["sequential_ID"] = df_relation_members.groupby([
                                                                       "relation_id"]).cumcount()  # Erstellung einer sequentiell ID (abzählen wie häufig eine relation nacheinander kommt, somit Anzahl der Member
...
df_way_nodes_alt_nt = df_way_nodes_alt["Nodes"].apply(pd.Series).reset_index().melt(id_vars="index").dropna()[
    ["index", "value"]].set_index("index")
df_way_nodes = pd.merge(df_way_nodes_alt["ID"], df_way_nodes_alt_nt, right_index=True, left_index=True).rename(
    columns={'value': 'node_id', 'ID': 'way_id'})
...
df_power_ways_alt["node_id"] = df_power_ways_alt["node_id"].astype(
    "str")  # Umschreiben der beiden IDs zu String, da eine Object und eine Integer ist.
df_nodes["node_id"] = df_nodes["node_id"].astype("str")
df_power_ways_alt = pd.merge(df_power_ways_alt, df_nodes, how="left",
                             on="node_id")  # Zusammenfügen der Ways und Nodes Tabelle um die Geometry über die Ways zu legen
df_power_ways_alt_2 = df_power_ways_alt[
    ["way_id", "geometry"]].copy()  # Erstellung der power_ways tabelle mit den Einzelnen Punkten der Geometrie
df_power_ways_alt_2 = df_power_ways_alt_2.rename(columns={"way_id": "ID"})  # rename way_id for adding new columns
df_power_ways = df_power_ways_alt_2.groupby(["ID"])["geometry"].agg(lambda x: LineString(
    x.tolist()))  # Die einzelnen Punkte werden zusammengefasst und zum Linestring umgewandelt für jeden Way
# df_power_ways=df_power_ways_alt_2.groupby("way_id").agg({"geometry":list})
df_power_way_columns = df_way_nodes_alt[
    ["ID", "power", "voltage", "cables", "wires", "circuits", "frequency", "name"]].copy()  # add needed columns
...
df_power_relations_new = df_power_relations[
    ["ID", 'voltage', "cables", "wires", "circuits", "frequency", "Nodes"]]  # drop unwanted columns
df_power_relations_new = df_power_relations_new.reindex(
    columns=["ID", 'voltage', "cables", "wires", "circuits", "frequency", "Nodes"])  # reorder columns
...
branch_data_columns = {"branch_id": [], 'relation_id': [], 'line_id': [], 'length': [], 'way geometry': [], 'f_bus': [],
                       't_bus': [], 'voltage': [], 'cables': [], 'wires': [], 'frequency': [], "power": []}
...
df_power_relations_applied_changes["frequency"] = df_power_relations_applied_changes["frequency"].astype(
    "str")  # change from float to string for the if function

df_power_relations_applied_changes["voltage"].loc[(df_power_relations_applied_changes["frequency"] == "50") & (
            df_power_relations_applied_changes["voltage"] == "400000")] = "380000"

#   Create table power_line     114-118
df_power_line = df_power_ways_applied_changes[df_power_ways_applied_changes.isin(["line", "cable"]).any(
    axis=1)]  # Select only the lines with power = cable or line
...
df_power_substation = df_power_ways_applied_changes[
    df_power_ways_applied_changes.isin(['substation', 'sub_station', 'station', 'plant']).any(
        axis=1)]  # select only lines where Power is 'substation','sub_station','station', 'plant'
for x in df_power_substation.index:
    if len(df_power_substation["geometry"][
               x].coords) < 4:  # delete geometry with less than 4 Points (you need 4 Points for a Polygon)
        df_power_substation.drop(index=x, inplace=True)

df_power_substation_geometry = df_power_substation["geometry"].agg(
    lambda x: Polygon(x))  # Create Polygon out of Linestrings
df_power_substation = gpd.GeoDataFrame(df_power_substation,
                                       geometry=df_power_substation_geometry)  # create the gpd with the Polygons
...
df_power_substation["Point"].loc[df_power_substation["point_intersect"] == True] = \
df_power_substation["var_result"].loc[df_power_substation.index]
...
df_power_line.drop(
    df_power_line.loc[df_power_line["point_substation_id_1"] == df_power_line["point_substation_id_2"]].index.to_list(),
    inplace=True)
...
#print(df_power_line["numb_volt_lev"])
#   277
...
for x in range((df_power_substation.voltage.str.count(";") + 1).max()):
    ...
    df_power_substation["voltage_array" + str(x)] = numbers_voltage_substation[x].loc[
        df_power_substation["int" + str(x)]]
    df_power_substation["voltage_array" + str(x)].loc[
        df_power_substation["voltage_array" + str(x)] == "60000"] = "110000"
...
df_power_substation["connection_110kV"].loc[df_power_line["point_within_start"].loc[
    (df_power_line["point_within_start"] >= 0) & (df_power_line["voltage_array1"] == "110000")]] = True
df_power_substation["connection_110kV"].loc[df_power_line["point_within_end"].loc[
    (df_power_line["point_within_end"] >= 0) & (df_power_line["voltage_array1"] == "110000")]] = True
    ...
    for x in df_power_line.cables:
        ...
        if "cable" in df_power_line["power"].values:
            if np.isnan(df_power_line.cables[x]) is True \
                ...
                match numb_volt_lev:
                    case 1:
                        df_power_line["cables_array"][1] = cables_per_circuit * change_datatype_semic(df_power_line,
                                                                                                      "circuits",
                                                                                                      1)
                    case numb_volt_lev if (
                                numb_volt_lev > 1 and numb_volt_lev == change_datatype_semic(df_power_line, "circuits",
                                                                                             1)):
    ...
    for x in df_power_line.cables:
        ...
        if "cable" in df_power_line["power"].values:
            if np.isnan(df_power_line.cables[x]) is True \
                    and np.isnan(df_power_line["circuits"][x]) is False and ";" not in df_power_line["circuits"][x] \
                    and (df_power_line["numb_volt_lev"][x] == 1 or
                         (df_power_line["numb_volt_lev"][x] == df_power_circuits.wires[x] and
                          len(df_power_line["frequency"])) == df_power_line["numb_volt_lev"][x]):
                match frequency_value:
                    case 50:
                        cables_per_circuit = 3
                ...
                match numb_volt_lev:
                    case 1:
                        df_power_line["cables_array"][1] = df_power_circuits.wires[x]
                    case numb_volt_lev if (
                                numb_volt_lev > 1 and numb_volt_lev == df_power_circuits.wires[x]):
                        for i in numb_volt_lev:
                            df_power_line["cables_array"][i] = cables_per_circuit
...
df_power_line["cables_from_neighbour"] = ""
df_power_line["frequency_from_neighbour"] = ""
df_power_line["cables_from_3_cables"] = ""
df_power_line["cables_from_sum"] = ""

# 479 unknown value heuristic
numb_unknown_cables_lev = ""
